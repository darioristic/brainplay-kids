...
const EnhancedParentDashboard: React.FC<Props> = ({
  ...
  const getWeeklyActivityData = () => {
    ...
    const avgEngagement = Math.min(
      100,
      (familyStats.weeklySessions / 7) * 10
    );
  ...
  const handleAddChild = async (newChild: ChildProfile) => {
    ...
    // Refresh stats after adding child
    await refreshStats();
  ...
  const handleUpdateChild = async (updatedChild: ChildProfile) => {
    ...
    // Refresh stats after updating child
    await refreshStats();
    // Call parent refresh if provided
    if (onRefresh) {
      onRefresh();
  ...
  const handleDeleteChild = async () => {
    ...
    try {
      ...
      // Refresh stats after deleting child
      await refreshStats();
      // Call parent refresh if provided
      if (onRefresh) {
        onRefresh();
    ...
    <div className="min-h-full bg-scandi-cream flex font-sans text-scandi-chocolate">
      ...
      >
        ...
        <nav className="flex-1 p-4 space-y-2">
          <a
            href="#"
            className="flex items-center gap-3 px-4 py-3 bg-scandi-oat text-scandi-chocolate rounded-xl font-bold transition"
          >
            <Layout size={18} /> Overview
          </a>
          <a
            href="#"
            className="flex items-center gap-3 px-4 py-3 text-scandi-stone hover:bg-scandi-cream hover:text-scandi-chocolate rounded-xl font-medium transition"
          >
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              ...
              <div
                key={i}
                className="bg-white p-6 rounded-[2rem] border border-scandi-oat shadow-soft flex flex-col justify-between h-40"
              >
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-scandi-cream/50 text-scandi-stone font-bold uppercase tracking-wider text-xs">
                  <tr>
                    ...
                    <th className="px-4 md:px-8 py-4 hidden sm:table-cell">
                      Mode
                    </th>
                    <th className="px-4 md:px-8 py-4">Points</th>
                    <th className="px-4 md:px-8 py-4 hidden md:table-cell">
                      Status
                    </th>
                ...
                <tbody className="divide-y divide-scandi-oat">
                  {childrenData.length === 0 ? (
                    <tr>
                      <td
                        colSpan={5}
                        className="px-4 md:px-8 py-12 text-center text-scandi-stone"
                      >
                    ...
                    childrenData.map((child) => {
                      ...
                      return (
                        <tr
                          key={child.id}
                          className="hover:bg-scandi-cream/30 transition"
                        >
      ...
      >
        <div className="space-y-4">
          <p className="text-scandi-chocolate">
            ...
            <span className="font-bold">{deletingChild?.name}</span>? This
            action cannot be undone and all their progress will be lost.
          </p>
